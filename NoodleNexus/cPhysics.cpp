...
void cPhysics::ProjectionOnAxis(glm::vec3 axis, sTriangle& triangle, float& minProj, float& maxProj)
{
...
bool cPhysics::CheckTriangleTriangleCollision(sTriangle& triA, sTriangle& triB)
{
...
//
//	// Copy that vector into the local mesh format 
//	for (std::vector<cVAOManager::sTriangle>::iterator itVAOTri = vecTriangles.begin();
//		itVAOTri != vecTriangles.end(); itVAOTri++)
//	{
//		cVAOManager::sTriangle curVAOTri = *itVAOTri;
//
//		sTriangle curTri;
...
//}
bool cPhysics::addTriangleMesh(
	const std::vector<cVAOManager::sTriangle>& vecVAOTriangles,
	glm::vec3 meshWorldPosition,
	glm::vec3 meshWorldOrientation,
	float uniformScale)
{
	if (vecVAOTriangles.empty())
	{
		// No triangles provided
		return false;
	}

	// At this point, vecVAOTriangles has all the triangle info.
	sTriangleMesh* pMesh = new sTriangleMesh();

	// Copy that vector into the local mesh format 
	for (std::vector<cVAOManager::sTriangle>::const_iterator itVAOTri = vecVAOTriangles.begin();
		itVAOTri != vecVAOTriangles.end(); ++itVAOTri)
	{
		cVAOManager::sTriangle curVAOTri = *itVAOTri;
		...
		itTri != pMesh->vecTriangles.end(); ++itTri)
	{
		...
		// Translation (movement, position, placement...)
		glm::mat4 matTranslate = glm::translate(glm::mat4(1.0f), meshWorldPosition);

		// Rotation
		glm::mat4 matRotateX = glm::rotate(glm::mat4(1.0f), glm::radians(meshWorldOrientation.x), glm::vec3(1.0f, 0.0f, 0.0f));
		glm::mat4 matRotateY = glm::rotate(glm::mat4(1.0f), glm::radians(meshWorldOrientation.y), glm::vec3(0.0f, 1.0f, 0.0f));
		glm::mat4 matRotateZ = glm::rotate(glm::mat4(1.0f), glm::radians(meshWorldOrientation.z), glm::vec3(0.0f, 0.0f, 1.0f));
		...
		for (int i = 0; i < 3; ++i)
		{
			itTri->vertices[i] = glm::vec3(matModel * glm::vec4(itTri->vertices[i], 1.0f));
		}

		// Transform normal
		glm::mat4 matModelInverseTranspose = glm::inverse(glm::transpose(matModel));
		itTri->normal = glm::vec3(matModelInverseTranspose * glm::vec4(itTri->normal, 1.0f));
	}

	// Assign a placeholder name if needed (optional)
	pMesh->meshInstanceName = "UnnamedMesh";
...
bool cPhysics::rayCastSpecificTriangles(glm::vec3 start, glm::vec3 end,
	std::vector<sCollision_RayTriangleInMesh>& vec_RayTriangle_Collisions, 
	std::vector<sTriangle> vecSpecificTriangles,
	bool bIgnoreBackFacingTriangles /*= true*/)
{
	sLine theRay;
	theRay.startXYZ = start;
	...
	// Go through each triangle
	for (std::vector<sTriangle>::iterator itTri = vecSpecificTriangles.begin();
		itTri != vecSpecificTriangles.end(); itTri++)
	{
...
//bool cPhysics::bLineSegment_TriangleCollision(sLine theLine, sTriangle theTri)
//{
//	// Use double for improved precision with small values
//	glm::dvec3 ab = theTri.vertices[1] - theTri.vertices[0]; // Vector ab = b - a;
//	glm::dvec3 ac = theTri.vertices[2] - theTri.vertices[0]; // Vector ac = c - a;
//	glm::dvec3 qp = theLine.startXYZ - theLine.endXYZ;       // Vector qp = p - q;
//
//	// Compute triangle normal
//	glm::dvec3 n = glm::cross(ab, ac); // Vector n = Cross(ab, ac);
//
//	// Use a small epsilon threshold for floating-point comparisons
//	const double EPSILON = 1e-8;
//
//	// Compute denominator d
//	double d = glm::dot(qp, n); // double d = Dot(qp, n);
//
//	// Parallel or pointing away
//	if (d > -EPSILON && d < EPSILON) { // Compare against small threshold
//		return false;
//	}
//
//	// Compute intersection t value
//	glm::dvec3 ap = theLine.startXYZ - theTri.vertices[0]; // Vector ap = p - a;
//	double t = glm::dot(ap, n);                            // t = Dot(ap, n);
//
//	if (t < -EPSILON) {
//		return false;
//	}
//
//	// For segment; exclude this line for ray test
//	if (t > d + EPSILON) {
//		return false;
//	}
//
//	// Compute barycentric coordinate components and test if within bounds
//	glm::dvec3 e = glm::cross(qp, ap); // Vector e = Cross(qp, ap);
//
//	double v = glm::dot(ac, e); // v = Dot(ac, e);
//	if (v < -EPSILON || v > d + EPSILON) {
//		return false;
//	}
//
//	double w = -glm::dot(ab, e); // w = -Dot(ab, e);
...
// Helper function to check overlap of two projection intervals
bool overlap(const std::pair<float, float>& proj1, const std::pair<float, float>& proj2) {
	return proj1.second >= proj2.first && proj2.second >= proj1.first;
}

std::pair<float, float> cPhysics::projectTriangleOntoAxis(const sTriangle& tri, const glm::vec3& axis) {
	float minProj = glm::dot(axis, tri.vertices[0]);
	float maxProj = minProj;
	for (int i = 1; i < 3; ++i) {
		float proj = glm::dot(axis, tri.vertices[i]);
		if (proj < minProj) {
			minProj = proj;
		}
		if (proj > maxProj) {
...
bool cPhysics::bTriangleTriangleCollision(const sTriangle& triA, const sTriangle& triB) {
	// Helper function for projecting a triangle onto an axis

	// Validate input normals
	if (glm::length(triA.normal) < 1e-6f || glm::length(triB.normal) < 1e-6f) {
		std::cerr << "Error: One or both triangles have invalid normals!" << std::endl;
		return false;
	}


	// Axes to test (normals of the two triangles)
	std::vector<glm::vec3> axes;
	axes.push_back(triA.normal);
	axes.push_back(triB.normal);

	// Add edge cross products as axes
	for (int i = 0; i < 3; ++i) {
 		for (int j = 0; j < 3; ++j) {
			glm::vec3 edgeA = triA.vertices[(i + 1) % 3] - triA.vertices[i];
			glm::vec3 edgeB = triB.vertices[(j + 1) % 3] - triB.vertices[j];
			glm::vec3 axis = glm::cross(edgeA, edgeB);
			if (glm::length(axis) > 1e-6f) { // Avoid degenerate axes
	...
	for (size_t axisIndex = 0; axisIndex < axes.size(); ++axisIndex) {
		glm::vec3 axis = axes[axisIndex];
		std::pair<float, float> projA = projectTriangleOntoAxis(triA, axis);
		std::pair<float, float> projB = projectTriangleOntoAxis(triB, axis);
