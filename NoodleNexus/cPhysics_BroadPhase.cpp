		...
		for (const cPhysics::sTriangle &curTessTriangle : vec_TessellatedTriangles)
		{
			...
			for (unsigned int vertIndex = 0; vertIndex < 3; vertIndex++)
			{
				...
				if (itAAAB_V0 == this->map_BP_CubeGrid.end())
				{
					...
					// Assuming vec_pTriangles is a vector of pointers to sTriangle
					//for (std::vector<sTriangle>::iterator it = pCube->vec_pTriangles.begin(); it != pCube->vec_pTriangles.end(); ++it) {
					//	if (&(*it) == &curTri) {
					//		// If the addresses match, curTri is already in the vector
					//		break;
					//	}
					//	else {
					//		pCube->vec_pTriangles.push_back(curTri);
					//	}
					//}
				}
