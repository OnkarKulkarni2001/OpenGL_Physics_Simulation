...
void sModelDrawInfo::calculateExtents(void)
{
	...
	// TODO: Anything else you need calculated
	 // Calculate the center of the bounding sphere (midpoint of min and max)
	this->boundingSphereCenter = glm::vec3(
		(this->minXYZ.x + this->maxXYZ.x) / 2.0f,
		(this->minXYZ.y + this->maxXYZ.y) / 2.0f,
		(this->minXYZ.z + this->maxXYZ.z) / 2.0f
	);

	// Calculate the radius of the bounding sphere (distance from center to a corner)
	glm::vec3 corner = glm::vec3(this->maxXYZ.x, this->maxXYZ.y, this->maxXYZ.z);
	this->boundingSphereRadius = glm::distance(this->boundingSphereCenter, corner);
