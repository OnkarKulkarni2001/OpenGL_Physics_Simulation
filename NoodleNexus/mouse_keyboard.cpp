...
//            ::g_cameraEye.y += CAMERA_MOVE_SPEED;
    ...
    if (areAllModifiersUp(window))
    {
        cPhysics::sPhysInfo* pEntityPhys = ::g_pPhysicEngine->pFindAssociateMeshByFriendlyName("xWing");

        const float ENTITY_SPEED_CHANGE = 1.0f;
        // Exists? 
        if (pEntityPhys)         // or != NULL
        {
            // CUT THE ENGINES!!!
            if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS)
            {
                pEntityPhys->velocity = glm::vec3(0.0f);
            }
            // Go forward
            if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
            {
                // Accelerate
                pEntityPhys->velocity.z += ENTITY_SPEED_CHANGE;
            }
            ...
            {
                // Accelerate
                pEntityPhys->velocity.x += ENTITY_SPEED_CHANGE;
            }
            if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
            {
                pEntityPhys->velocity.x -= ENTITY_SPEED_CHANGE;
            ...
            if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS)
            {
                pEntityPhys->velocity.y += ENTITY_SPEED_CHANGE;
            }
            if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS)
            {
                pEntityPhys->velocity.y -= ENTITY_SPEED_CHANGE;
